...
`harbor-mob-offline-lens` is centered on mobile workflows work in Go. The technical target is: Create a Go reference implementation for offline workflows, centered on stream reduction, windowed input fixtures, and late-data behavior checks.
...
The score is deterministic. The current threshold is 175. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 5.
